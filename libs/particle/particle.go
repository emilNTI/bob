...
func (p *ParticleEmitter) AddParticle(x_spread, y_spread float32, lifetime_spread int, col color.RGBA) {
	t := Particle{}
	t.dead = false
...
func (p *ParticleEmitter) GetPosition() Vec2f {
	return p.position
}

func (p *ParticleEmitter) GetCurrentParticles() int { return len(p.particles) }
